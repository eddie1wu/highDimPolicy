...
def run_one_replication(
    ...
    beta_0 = rng.normal(-1.0, 2.0, size=x_dim_max)
    beta_1 = rng.normal(1.0, 2.0, size=x_dim_max)
    ...
    W_full = rng.normal(0, 1 / np.sqrt(X_tr.shape[1]), size=(x_dim_max, n_dim_max))
    ...
    for k in tqdm(dim_grid):
        Xtr_k = np.maximum(0, X_tr @ W_full[:, :k])
        Xte_k = np.maximum(0, X_te @ W_full[:, :k])
