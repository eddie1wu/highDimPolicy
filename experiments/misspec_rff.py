...
def main():
    ...
    cfg = load_config("configs/misspec_rff.toml")

    results = run_rff(cfg)

    run_dir = make_run_dir(Path("results"), tag="misspec_rff")
...
def run_rff(cfg, max_workers: int | None = None):
...
def run_single_trial(cfg, trial_idx: int = 0):
    ...
    # Create random Fourier sieves
    W = rng.normal(
        loc = 0.0,
        scale = 1.0,
        size = (cfg.X_dim_max, cfg.dim_max)
    )
    ...
    # Phi = np.sqrt(2.0 / cfg.dim_max) * np.cos( X @ W + b )
    ...
    for idx in tqdm(range(len(p_grid))):
        ...
        Phi = np.sqrt(2.0 / p) * np.cos( X @ W[:, :p] + b[:p] )
        Phi_train_curr, Phi_test_curr = split_train_test(cfg.n_train, Phi)
        ...
        # can_interpolate = check_interpolation(Phi_train_curr, target_train, atol = 0.0)
        # print(f"Interpolation for feature dimension {p}: {can_interpolate}.")
