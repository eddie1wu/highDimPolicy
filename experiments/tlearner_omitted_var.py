...
def main():
    ...
    cfg = load_config("configs/tlearner_omitted_var.toml")

    results = run_tlearner_omitted_var(cfg)

    run_dir = make_run_dir(Path("results"), tag="tlearner_omitted_var")
...
def run_tlearner_omitted_var(cfg, max_workers: int | None = None):
    ...
    tl_train_welfare, tl_test_welfare, clf_train_welfare, clf_test_welfare, \
        sb_oracle_welfare, oracle_train_welfare, oracle_test_welfare, \
...
def run_single_trial(cfg, trial_idx: int = 0):
    ...
    for idx in tqdm(range(len(p_grid))):
        ...
        Phi_train_curr, Phi_test_curr = X_train[:, :p], X_test[:, :p]
        ...
        ### Second best oracle
        sb_oracle_welfare[idx] = compute_welfare(Phi_test_curr @ (beta_1[:p] - beta_0[:p]), Y0_test, Y1_test)
