...
def main():
    ...
    res_path = Path("results/2026-03-30_235030_random_ReLU_intercept")
    res = load_results(res_path)

    fig1, fig2, fig3, fig4 = plot_graphs(res)

    out_path = Path(f"graphs/welfare_random_ReLU_{timestamp}.png")
    save_fig(fig1, out_path)

    out_path = Path(f"graphs/shrinkage_random_ReLU_{timestamp}.png")
    save_fig(fig2, out_path)

    out_path = Path(f"graphs/risk_random_ReLU_{timestamp}.png")
    save_fig(fig3, out_path)

    out_path = Path(f"graphs/norm_random_ReLU_{timestamp}.png")
...
def plot_graphs(
    ...
    original_X_welfare = res["original_X_welfare"]
    ...
    p_grid = res["p_grid"]
    interpolating_threshold = 65

    ### Training test welfare
    title = "Double Ascent in Welfare Maximization (Random ReLU Features)"
    fig1, ax = new_fig()
    ...
    ax.axhline(original_X_welfare, ls = "--", label = "Using original X's", color = "C0")
    ...
    title = "Comparing to Shrinkage Estimators (Random ReLU Features)"
    fig2, ax = new_fig()
    ...
    ### Risk plot
    title = "Training loss and risk (Random ReLU Features)"
    fig3, ax = new_fig()
    ...
    title = "Norm of fitted parameter vector (Random ReLU Features)"
    fig4, ax = new_fig()
