...
def main():
    ...
    res_path = Path("results/2026-04-01_031233_factor_X")
    res = load_results(res_path)

    fig1, fig2, fig3, fig4 = plot_graphs(res)

    out_path = Path(f"graphs/welfare_factor_X_{timestamp}.png")
    save_fig(fig1, out_path)

    out_path = Path(f"graphs/shrinkage_factor_X_{timestamp}.png")
    save_fig(fig2, out_path)

    out_path = Path(f"graphs/risk_factor_X_{timestamp}.png")
    save_fig(fig3, out_path)

    out_path = Path(f"graphs/norm_factor_X_{timestamp}.png")
...
def plot_graphs(
    ...
    title = "Double Ascent in Welfare Maximization (Factor X)"
    fig1, ax = new_fig()
    ...
    title = "Comparing to Shrinkage Estimators (Factor X)"
    fig2, ax = new_fig()
    ...
    ### Risk plot
    title = "Training Loss and Risk (Factor X)"
    fig3, ax = new_fig()
    ...
    title = "Norm of fitted parameter vector (Factor X)"
    fig4, ax = new_fig()
