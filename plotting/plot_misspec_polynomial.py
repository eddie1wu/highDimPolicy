...
def main():
    ...
    res_path = Path("results/2026-03-31_144406_misspec_polynomial")
    res = load_results(res_path)

    fig1, fig2, fig3 = plot_graphs(res)

    out_path = Path(f"graphs/welfare_misspec_pn_{timestamp}.png")
    save_fig(fig1, out_path)

    out_path = Path(f"graphs/risk_misspec_pn_{timestamp}.png")
    save_fig(fig2, out_path)

    out_path = Path(f"graphs/norm_misspec_pn_{timestamp}.png")
...
def plot_graphs(
    ...
    title = "Welfare Double Ascent under Model Misspecification (Polynomial Sieves)"
    fig1, ax = new_fig()
    ...
    ### Risk plot
    title = "Training loss and risk under Model Misspecification (Polynomial Sieves)"
    fig2, ax = new_fig()
    ...
    title = "Norm of fitted parameter vector (Polynomial Sieves)"
    fig3, ax = new_fig()
