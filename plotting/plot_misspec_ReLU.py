...
def main():
    ...
    res_path = Path("results/2026-03-31_121203_misspec_random_ReLU")
    res = load_results(res_path)

    fig1, fig2, fig3 = plot_graphs(res)

    out_path = Path(f"graphs/welfare_misspec_ReLU_{timestamp}.png")
    save_fig(fig1, out_path)

    out_path = Path(f"graphs/risk_misspec_ReLU_{timestamp}.png")
    save_fig(fig2, out_path)

    out_path = Path(f"graphs/norm_misspec_ReLU_{timestamp}.png")
...
def plot_graphs(
    ...
    p_grid = res["p_grid"]
    interpolating_threshold = 60


    ### Training test welfare
    title = "Welfare Double Ascent under Model Misspecification (Random ReLU Sieves)"
    fig1, ax = new_fig()
    ...
    ### Risk plot
    title = "Training loss and risk under Model Misspecification (Random ReLU Sieves)"
    fig2, ax = new_fig()
    ...
    title = "Norm of fitted parameter vector (Random ReLU Sieves)"
    fig3, ax = new_fig()
