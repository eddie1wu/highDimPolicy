...
def main():
    ...
    res_path = Path("results/2026-03-30_083154_omitted_var")
    res = load_results(res_path)

    fig1, fig2, fig3, fig4 = plot_graphs(res)

    out_path = Path(f"graphs/welfare_omitted_var_{timestamp}.png")
    save_fig(fig1, out_path)

    out_path = Path(f"graphs/shrinkage_omitted_var_{timestamp}.png")
    save_fig(fig2, out_path)

    out_path = Path(f"graphs/risk_omitted_var_{timestamp}.png")
    save_fig(fig3, out_path)

    out_path = Path(f"graphs/norm_omitted_var_{timestamp}.png")
...
def plot_graphs(
    ...
    p_grid = res["p_grid"]
    interpolating_threshold = 75

    ### Training test welfare
    title = "Double Ascent in Welfare Maximization (Omitted Variables)"
    fig1, ax = new_fig()
    ax.plot(p_grid, train_welfare, label = "Training welfare", color = "Orange")
    ax.plot(p_grid, test_welfare, label = "Test welfare", color = "C0")
    ax.plot(p_grid, sb_oracle_welfare, ls="--", label="Second best oracle", color="C0")
    ...
    title = "Comparing to Shrinkage Estimators (Omitted Variables)"
    fig2, ax = new_fig()
    ...
    ### Risk plot
    title = "Training loss and risk (Omitted Variables)"
    fig3, ax = new_fig()
    ...
    title = "Norm of fitted parameter vector (Omitted Variables)"
    fig4, ax = new_fig()
