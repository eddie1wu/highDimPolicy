...
def main():
    ...
    res_path = Path("results/2026-03-31_160531_tlearner_ReLU")
    res = load_results(res_path)

    fig1, fig2 = plot_graphs(res)

    out_path = Path(f"graphs/tl_ReLU_welfare_{timestamp}.png")
    save_fig(fig1, out_path)

    out_path = Path(f"graphs/tl_ReLU_risk_{timestamp}.png")
...
def plot_graphs(
    ...
    tl_original_X_welfare = res["tl_original_X_welfare"]
    clf_original_X_welfare = res["clf_original_X_welfare"]
    ...
    title = "Welfare Performance of T-learner vs Weighted Classification (Random ReLU Model)"
    fig1, ax = new_fig()
    ...
    ax.axhline(tl_original_X_welfare, ls = "--", label = "TL Original X", color = "Orange")
    ax.axhline(clf_original_X_welfare, ls="--", label="Clf Original X", color="C0")
    ...
    ### Risk plot
    title = "Training Loss and Risk (Random ReLU Model)"
    fig2, ax = new_fig()
