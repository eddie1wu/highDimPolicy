...
def main():
    ...
    res_path = Path("results/2026-03-31_192235_tlearner_misspec_ReLU")
    res = load_results(res_path)

    fig1, fig2 = plot_graphs(res)

    out_path = Path(f"graphs/tl_misspec_ReLU_welfare_{timestamp}.png")
    save_fig(fig1, out_path)

    out_path = Path(f"graphs/tl_misspec_ReLU_risk_{timestamp}.png")
...
def plot_graphs(
    ...
    title = "Welfare Performance of T-learner vs Weighted Classification (ReLU Sieves)"
    fig1, ax = new_fig()
    ...
    ### Risk plot
    title = "Training Loss and Risk (ReLU Sieves)"
    fig2, ax = new_fig()
